...
namespace AdventOfCode.Day07
{
    public static class Main
    {
        public static void Run()
        {
            ...
            var needles = new List<string> {"shiny gold"};
            while (needles.Count > 0)
            {
                needles = rules
                    .Where(r => r.Value.Any(x => needles.Any(y => y == x.Item1)))
                    .Select(r => r.Key)
                    .Where(k => !bags.Contains(k)).ToList();
                bags.AddRange(needles);
            }
            Console.WriteLine($"There are {bags.Distinct().Count()} bags you could carry");
        ...
        private static int CountBagsInside(string key, Dictionary<string,List<Tuple<string, int>>> rules)
        {
            ...
            return bag.Select(t => t.Item2 *(1+CountBagsInside(t.Item1, rules))).Aggregate((a, b) => a + b);
        }

        private static Dictionary<string, List<Tuple<string, int>>> InitRules()
        {
            ...
            foreach (var rule in data)
            {
                ...
                var children = ChildrenParser.Matches(rule);

                dict.Add(key.Groups[1].Value, children.Select(m => new Tuple<string, int>(m.Groups[2].Value, short.Parse(m.Groups[1].Value))).ToList());
            }

            return dict;
        }
        private static Regex KeyParser = new Regex(@"(^[a-z\s]+)\sbags\scontain");
        private static Regex ChildrenParser = new Regex(@"([0-9])\s([a-z\s]+) bags?[,\.]");
