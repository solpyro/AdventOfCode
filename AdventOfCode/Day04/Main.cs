...
namespace AdventOfCode.Day04
{
    public static class Main
    {
        ...
        {
            var validData = data.Where(IsPolarId).ToList();
            Console.WriteLine($"There are {validData.Count()} valid IDs");
            Console.WriteLine($"There are {validData.Count(IsValidatedPolarId)} validated IDs");
        }
        ...
        private static bool IsValidatedPolarId(string id)
        {
            if (PidV.IsMatch(id) &&
                HclV.IsMatch(id) &&
                EclV.IsMatch(id))
        ...
        private static bool IsValidByr(string id)
        {
            var match = ByrV.Match(id);

            if (match.Success)
            {
                return InRange(match.Groups[1].Value, 1920, 2002);
            }

            return false;
        }
        private static bool IsValidIyr(string id)
        {
            var match = IyrV.Match(id);

            if (match.Success)
            {
                return InRange(match.Groups[1].Value, 2010, 2020);
            }

            return false;
        }
        private static bool IsValidEyr(string id)
        {
            var match = EyrV.Match(id);

            if (match.Success)
            {
                return InRange(match.Groups[1].Value, 2020, 2030);
            }

            return false;
        }
        private static bool IsValidHgt(string id)
        {
            var match = HgtV.Match(id);
            ...
            if (match.Success)
            {
                ...
                {
                    case "cm":
                        return InRange(match.Groups[1].Value,150,193);
                    case "in":
                        return InRange(match.Groups[1].Value, 59, 76);
                }
            }

            return false;
        }
        private static bool InRange(string sNum, int min, int max)
        {
            var num = short.Parse(sNum);
        ...
        private static Regex ByrV = new Regex(@"byr:([0-9]{4})(?:$|\s)");
        private static Regex IyrV = new Regex(@"iyr:([0-9]{4})(?:$|\s)");
        private static Regex EyrV = new Regex(@"eyr:([0-9]{4})(?:$|\s)");
        private static Regex HgtV = new Regex(@"hgt:([0-9]+)(cm|in)");
        private static Regex HclV = new Regex(@"hcl:#[0-9a-f]{6}(?:$|\s)");
        private static Regex EclV = new Regex(@"ecl:(?:amb|blu|brn|gry|grn|hzl|oth)");
        private static Regex PidV = new Regex(@"pid:[0-9]{9}(?:$|\s)");
