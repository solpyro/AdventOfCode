...
FireBeam { Cell=(0,0); Direction=East }
|> printfn "Part 1: %i"

[|
    for x in [0..(grid.GetLength 1)-1] do
        yield { Cell=(x,0); Direction=South }
        yield { Cell=(x,(grid.GetLength 0)-1); Direction=North }
    for y in [0..(grid.GetLength 0)-1] do
        yield { Cell=(0,y); Direction=East }
        yield { Cell=((grid.GetLength 1)-1,y); Direction=West }
|]
|> Array.map FireBeam
